...
public class FieldParser : FieldParserBase
{
            ...
            catch (Exception ex) when (!(ex is FieldParsingException))
            {
                if (options.ValidateFields)
                {
                    throw new FieldParsingException(
                        field,
                        data,
                        $"Failed to parse {field.Type} field '{field.Name}': {ex.Message}",
                        ex
                    );
                }
    ...
    )
    {
        ...
        if (field.ActualDecimalCount == 0)
        {
            if (
                int.TryParse(
                    text,
                    NumberStyles.Integer | NumberStyles.AllowParentheses,
                    CultureInfo.InvariantCulture,
                    out var intResult
                )
            )
        ...
        // Try decimal with support for parentheses (accounting format)
        if (
            decimal.TryParse(
                text,
        ...
        // Debug output for cp1251.dbf issue
        if (text.Contains("db.d"))
        {
    ...
    )
    {
        ...
        var julianDay = BitConverter.ToUInt32(data[..4]);
        ...
        catch (ArgumentException)
        {
            throw new FormatException(
                $"Invalid timestamp: Julian day {julianDay}, milliseconds {milliseconds}"
            );
        }
    }
    ...
    /// </summary>
    private object? ParseAutoincrement(
        DbfField field,
        ReadOnlySpan<byte> data,
        Encoding encoding,
        DbfReaderOptions options
